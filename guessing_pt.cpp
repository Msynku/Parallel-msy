...
#ifndef PT_BATCH_THREADS
#define PT_BATCH_THREADS 4
#endif
...
void PriorityQueue::PopNext()
{
    ...
    // 按概率把新的PT插回优先队列，保持 priority 按 prob 降序排列
void PriorityQueue::InsertPT(PT pt)
{
    CalProb(pt);
    ...
    while (iter != priority.end() && iter->prob >= pt.prob)
    {
        iter++;
    }
...
// 生成一个PT对应的guesses，但不写入成员变量 guesses
// 这样多个PT可以并行生成，各自写入自己的局部vector
void PriorityQueue::GenerateToVector(PT pt, vector<string>& out)
{
    ...
    else
    {
        ...
        for (int i = 0; i < total; i++)
        {
            out[i] = prefix + a->ordered_values[i];
        }
    }
}

// PT批处理版本：一次从优先队列取多个PT，多个PT并行生成
void PriorityQueue::PopNextBatch(int batch_size)
{
    ...
    if (batch > (int)priority.size())
    {
        batch = priority.size();
    }

    // 一次性取出队首的多个PT
    vector<PT> batch_pts(priority.begin(), priority.begin() + batch);

    // 先从优先队列删除这一批PT
    // 等所有PT生成完成后，再统一插入新PT
    priority.erase(priority.begin(), priority.begin() + batch);

    vector<vector<string> > local_guesses(batch);
    vector<vector<PT> > local_new_pts(batch);

    // PT层面的并行：不同线程处理不同PT
#pragma omp parallel for num_threads(PT_BATCH_THREADS) schedule(dynamic)
    for (int i = 0; i < batch; i++)
    {
        GenerateToVector(batch_pts[i], local_guesses[i]);
        local_new_pts[i] = batch_pts[i].NewPTs();
    }
    ...
    for (int i = 0; i < batch; i++)
    {
        ...
        total_guesses += local_guesses[i].size();
    }

    // 统一把新PT插回优先队列
    for (int i = 0; i < batch; i++)
    {
        for (PT pt : local_new_pts[i])
        {
            InsertPT(pt);
        }
    }
